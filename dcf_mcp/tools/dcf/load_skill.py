...
def load_skill(skill_manifest: str, agent_id: str) -> Dict[str, Any]:
    ...
    The loader merges the historical ``load_skill`` and
    ``load_skill_with_resolver`` behaviours into a single tool while expanding the
    accepted manifest formats. ``skill_manifest`` may now be either the literal
    JSON content of a manifest **or** a filesystem path/URI pointing to the JSON
    file. When a path is supplied it may be absolute, relative to the working
    directory, or prefixed with ``file://``.
