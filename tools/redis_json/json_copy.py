...
def json_copy(
    ...
    if fp == "$" or tp == "$":
        return {"success": False, "error": "Copying root is not supported.", "redis_key": redis_key, "doc_json": None}
    ...
    for part in tparts[:-1]:
        if not isinstance(cur, dict):
            return {"success": False, "error": "Destination parent is not an object.", "redis_key": redis_key, "doc_json": None}
