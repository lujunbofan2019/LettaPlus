...
def json_move(
    ...
    *,
    ...
    """
    Move a JSON subtree from `from_path` to `to_path` inside a RedisJSON document, atomically.
    ...
    Args:
        ...
        from_path (str): Source JSONPath to move from (e.g., $.a.b).
        to_path (str): Destination JSONPath to move to (e.g., $.x.y).
        ...
        require_dest_parent_exists (bool): If True, error if destination parent chain is missing. Default False.
        ...
        subtree_schema_json (str): JSON Schema (as JSON string) for the moved subtree at `to_path`.
    ...
    if fp == "$" or tp == "$":
        return {"success": False, "error": "Moving the root or into the root '$' is not supported.", "doc": None}
    ...
    if len(to_keys) >= len(from_keys) and from_keys == to_keys[:len(from_keys)]:
        return {"success": False, "error": "Cannot move a node into its own descendant.", "doc": None}
    ...
    while True:
        ...
        try:
            ...
            # 2) Locate source parent and value
            cur_src = doc
            ...
            # 5) Perform move
            cur_dst[dest_leaf] = moved_value
            del cur_src[src_leaf]
                ...
                if ap.startswith("$"):
                    ...
                    if not bad_audit:
                        ...
                        if ok and isinstance(cur_a, list):
                            ...
                            cur_a.append({"ts": ts, "op": "move", "from": from_path, "to": to_path})
