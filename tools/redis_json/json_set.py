...
def json_set(
    ...
    *,
    ...
    validation_scope: str = "subtree",    # one of: "none", "subtree", "document"
    ...
    """
    Set a value at a JSON object path inside a RedisJSON document, atomically,
    with optional jsonschema validation and lightweight audit logging.
    ...
    Args:
        ...
        path (str): JSONPath to set (e.g., $.a.b or $["a"]["b"]). Root "$" replaces the entire doc.
        value_json (str): The value to set, encoded as a JSON string (e.g., '"hello"', '123', 'true', 'null', '{}', '[]').
    ...
    """
    # Parse inputs that arrive as strings
    try:
    ...
    while True:
        ...
        try:
            ...
            # 2) Parse path to keys (supports $.a.b and $["a"]["b"])
            p = path.strip()
            if not p.startswith("$"):
            ...
            # 3) If require_exists, check presence
            if require_exists:
                cur = doc
                if len(keys) == 0:
            ...
            # 4) Mutate in-memory (create missing containers if allowed)
            if len(keys) == 0:
                if isinstance(value, dict):
                    if isinstance(doc, dict):
                        doc.clear(); doc.update(value)
                    else:
                        return {"success": False, "error": "Root replace requires object document.", "doc": None}
                else:
                    return {"success": False, "error": "Root must be an object for this setter.", "doc": None}
            else:
                cur = doc
                for k in keys[:-1]:
            ...
            if validation_scope == "subtree" and subtree_schema is not None:
                ...
                for k in keys:
                    if not isinstance(target, dict) or k not in target:
                        ...
                        break
                    target = target[k]
                Draft202012Validator(subtree_schema).validate(target)
                ...
                if ap.startswith("$"):
                    ...
                    if not bad_audit:
                        ...
                        if ok and isinstance(cur2, list):
                            ...
                            cur2.append({"ts": ts, "op": "set", "path": path})
